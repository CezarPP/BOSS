...
namespace std {
    ...
    /**
     * A string of the given character type.
    ...
    public:
        ...
        /**
         * Construct a new string from the given raw string
         */
        basic_string(const CharT *s) : size_(strlen(s)) {
        ...
        /**
         * Construct a new string of the given capacity.
         */
        explicit basic_string(size_t capacity) : size_(0) {
            set_small(capacity <= 16);
        ...
        /**
         * Construct a new string from the given range of characters
         */
        template<typename It>
        basic_string(It it, It end) {
            size_ = std::distance(it, end);
        ...
        basic_string(const basic_string &rhs) : size_(rhs.size_) {
            if (!is_small()) {
        ...
        basic_string(basic_string &&rhs) : size_(rhs.size_) {
            if (is_small()) {
        ...
        [[nodiscard]] const CharT *data_ptr() const {
            if (is_small()) {
                return &storage.small.data[0];
            } else {
                return storage.big.data.get();
         ...
         */
        [[nodiscard]] int compare(const basic_string &rhs) const noexcept {
    ...
    template<typename Char>
    void
    split_append(const std::basic_string<Char> &s, std::vector<std::basic_string<Char>> &container, char sep = ' ') {
        std::basic_string<Char> current(s.size());
